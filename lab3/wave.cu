...
__global__ void wave_gpu_naive_step(
    ...
    for (int32_t idx_y = blockDim.y * blockIdx.y + threadIdx.y; idx_y < n_cells_y; idx_y += gridDim.y * blockDim.y) {
        for (int32_t idx_x = blockDim.x * blockIdx.x + threadIdx.x; idx_x < n_cells_x; idx_x += gridDim.x * blockDim.x) {
            int32_t idx = idx_y * n_cells_x + idx_x; // Okay so this says, nth y-index so we have n rows of x cells, then current x-index. Can i parallelize by row or not?
            bool is_border = 
                (idx_x == 0 || idx_x == n_cells_x - 1 || idx_y == 0 || idx_y == n_cells_y - 1);
            float u_next_val;
            if (is_border || Scene::is_wall(idx_x, idx_y)) { // Scene used to define the particulars.
                u_next_val = 0.0f;
            } else if (Scene::is_source(idx_x, idx_y)) {
                u_next_val = Scene::source_value(idx_x, idx_y, t);
            } else {
...
std::pair<float *, float *> wave_gpu_naive(
    ...
    dim3 num_blocks = dim3((Scene::n_cells_x + 15) / 16, (Scene::n_cells_y + 15) / 16);
...
////////////////////////////////////////////////////////////////////////////////
// TEST: GPU naive without stride loops — one thread per cell, grid sized to cover domain exactly.

// template <typename Scene>
// __global__ void wave_gpu_noloop_step(float t, float *u0, float const *u1) {
//     constexpr int32_t n_cells_x = Scene::n_cells_x;
//     constexpr int32_t n_cells_y = Scene::n_cells_y;
//     constexpr float c = Scene::c;
//     constexpr float dx = Scene::dx;
...
//     // One thread = one cell. No loop.
//     int32_t idx_x = blockDim.x * blockIdx.x + threadIdx.x;
//     int32_t idx_y = blockDim.y * blockIdx.y + threadIdx.y;

//     // Grid is rounded up so some threads fall outside the domain — skip them.
...
//             ((2.0f - damping - 4.0f * coeff) * u1[idx] -
//              (1.0f - damping) * u0[idx] +
//              coeff * (u1[idx - 1] + u1[idx + 1] + u1[idx - n_cells_x] + u1[idx + n_cells_x]));
//     }
//     u0[idx] = u_next_val;
// }

// template <typename Scene>
// std::pair<float *, float *> wave_gpu_naive(float t0, int32_t n_steps, float *u0, float *u1) {
//     // Launch exactly enough blocks to cover every cell — ceil(n_cells / block_dim) in each axis.
//     dim3 block_size(16, 16);
//     dim3 num_blocks((Scene::n_cells_x + 15) / 16, (Scene::n_cells_y + 15) / 16);
//     for (int32_t step = 0; step < n_steps; ++step) {
//         float t = t0 + step * Scene::dt;
//         wave_gpu_noloop_step<Scene><<<num_blocks, block_size>>>(t, u0, u1);
//         std::swap(u0, u1);
//     }
//     return {u0, u1};
// }
