...
__global__ void mandelbrot_gpu_vector_ilp(
    ...
    for (uint64_t i = 0; i < img_size; ++i) {
        for (uint64_t j = threadIdx.x; j < img_size; j += 32 * N) {
            // Get the plane coordinate X for the image pixel.
            float cx[N], x2[N], y2[N], w[N];
            uint32_t iters[N];
            bool active[N];

            #pragma unroll
            for (int k = 0; k < N; ++k) { // Initialize all except cy, we are keeping columns constant here.
                if (j + 32 * k < img_size) {
                    cx[k] = (float(j + 32 * k) / float(img_size)) * window_zoom + window_x;
                    x2[k] = 0.0f; y2[k] = 0.0f; w[k] = 0.0f;
                    iters[k] = 0;
                    active[k] = true;
                } else {
                    active[k] = false;
            ...
            bool flag = true; // flag set if any of a thread are still doing.
            while (flag) {
                flag = false;
                #pragma unroll 
                for (int k = 0; k < N; ++k) {
                    if (active[k]) {
                        ...
                        float z = x + y; w[k] = z * z;
                        ++iters[k];
                        active[k] = (x2[k] + y2[k] <= 4.0f) && (iters[k] < max_iters);
                    }
                    flag |= active[k];
                }
            }
...
void launch_mandelbrot_gpu_vector_ilp(
    ...
    int M = 1;
    int N = 32;
    mandelbrot_gpu_vector_ilp<<<M, N>>>(img_size, max_iters, out);
}
...
__global__ void mandelbrot_gpu_vector_multicore(
    ...
    uint32_t num_blocks = gridDim.x; // get the number of blocks
    uint32_t threads_per_block = blockDim.x;
    constexpr int N = 4; //
    for (uint64_t i = blockIdx.x; i < img_size; i += num_blocks) { // strategy: assign a block to a row
        for (uint64_t j = threadIdx.x; j < img_size; j += threads_per_block * N) {
            // Get the plane coordinate X for the image pixel.
            float cx[N], x2[N], y2[N], w[N];
            uint32_t iters[N];
            bool active[N];
            ...
            #pragma unroll
            for (int k = 0; k < N; ++k) { // Initialize all except cy, we are keeping columns constant here.
                if (j + threads_per_block * k < img_size) {
                    cx[k] = (float(j + threads_per_block * k) / float(img_size)) * window_zoom + window_x;
                    x2[k] = 0.0f; y2[k] = 0.0f; w[k] = 0.0f;
                    iters[k] = 0;
                    active[k] = true;
                } else {
            ...
            for (int k = 0; k < N; ++k) {
                if (j + threads_per_block * k < img_size) {
                    out[i * img_size + j + threads_per_block * k] = iters[k];
                }
            }
        }
    }
...
void launch_mandelbrot_gpu_vector_multicore(
    ...
    int M = 48;
    int N = 4 * 32;
    mandelbrot_gpu_vector_multicore<<<M, N>>>(img_size, max_iters, out);
}
