...
__global__ void mandelbrot_gpu_scalar(
    ...
    // __global__ refers to CUDA kernel that runs on GPU but is launched on GPU.
...
void launch_mandelbrot_gpu_scalar(
    ...
    /* your (CPU) code here... */
    // CPU launches with syntax kernel_name<<<N, m>>>(arg_1,...,arg_n)
    // N: numBlocks
    // M: threadsPerBlock
    // NB: I'm sure this textbook will have good guidance on how to intelligently set these.
...
void launch_mandelbrot_gpu_vector(
    ...
    int M = 1; // blocks
    int N = 32; // threads
    mandelbrot_gpu_vector<<<M, N>>>(img_size, max_iters, out);
}
